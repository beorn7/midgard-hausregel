...
\title{Hausregeln f"ur
  \textsc{Midgard} (4. Auflage)\\Version 2.8}
...
da"s der Spielleiter immer recht hat und erhaben "uber jede Regel ist
...
\item \emph{\textbf{Schleichen}} [DFR\,170]: Das Regelwerk sieht
  ...
  automatisch kritisch getroffen (bzw. ohne Zeitdruck und bei
  ungestörtem Angreifer sogar tödlich) recht leicht herbeiführbar
...
rechtfertigt. Je nach Situation kann er dabei sogar nach einen
mißlungenen EW einen PP vergeben ("`auch aus Fehlern kann man lernen"'
oder auch im Sinne der Variante für Wissensfertigkeiten
[DFR\,278]). Die Wahrscheinlichkeiten, die sich durch die Anwendung
der Standardregeln für die PP-Vergabe ergeben hätten, können dabei
durchaus eine Richtlinie sein (einem Expertes bringt eine einfach
Routineaufgabe kaum noch Praxiserfahrung), aber die letztendliche
Kontrolle bleibt beim Spielleiter.
...
wiederholen, werden immer \emph{beide} EW (nach Einsetzem eines
Punktes SG) wiederholt.
...
Während der Bewedung sind \textbf{Drehungen} im Prinzip
...
Handlungen werden (zumindestens im Prinzip) \textbf{gleichzeitig
  angesagt} und dann in \textbf{Handlungs-
  bzw. Angriffsrangreihenfolge durchgef"uhrt}. Eine angesagte Handlung
kann im Nachhinein \textbf{nicht mehr ge"andert} werden (mit der
Ausnahme einiger spezieller Situationen, in denen die Regeln dies
...
ist. (Manchmal hat man auch gar keine andere Wahl\dots)

Wird ein Effekt ausgelöst, der eine bestimmte Zeitdauer anhält, so
wird diese in der Regel erst vom Beginn der nächsten Runde an
gemessen, auch wenn der Effekt sofort einsetzt. Wird eine Figur also
beispielsweise für eine Runde bewußtlos, so ist sie für den Rest der
laufenden Runde und für die gesamte folgende Runde ausgeschaltet.

Beide Anmerkungen unterstreichen die Wichtigkeit von Angriffs-
...
Ausnahme darstellen. Dem Spielleiter sollte klar sein, daß sie ihm
...
können je nach Situation (Spielleiterentscheidung) bereitgelegt oder
...
% Verkürzung der Heildauer schwerer Verletzungen entfernt da
% inkompatibel mit Wundrune.
...
vergeben werden als wenn der Gegner "`kampflos"' überwunden wurde.
...
sogar bei einem kritischen Erfolg, der die in [DFR\,250] beschrieben
...
augenblicklick t"odlich sein kann, wenn prinzipiell die Möglichkeit
...
richtet "`nur"' passenden kritischen Schaden an (im Geiste des
Beispiels [DFR\,250]).
...
ihre Fähigkeit zur \emph{Abwehr} aussaget (was ja formal keine
Handlung ist). Die Situation eines Festgehaltenen entspricht der Stufe
2a in der Tabelle zur Handlungsunfähigkeit am Ende dieser Hausregel.
...
\subsubsection{Kampf ohne Waffen}

Der Zuschlag zum WW:Abwehr beim \emph{waffenlosen Kampf} wird nur in
Runden angerechnet, in denen der Kämpfer diese Waffenfertigkeit auch
wirklich anwendet. Letzteres muß sich nicht unbedingt durch einen
tatsächlichen EW:Angriff mit \emph{waffenlosen Kampf} definieren und
kann vom Spielleiter durchaus flexibel gehandhabt werden. Der Zuschlag
kommt aber insbesondere dann nicht zur Geltung, wenn der Kämpfer noch
eine Waffe in der Hand hält, sich in der laufenden Runde um mehr als
die Hälfte seiner Bewegungsweite bewegt hat, \emph{Faustkampf}
betreibt oder sich einer gänzlich anderen Tätigkeit widmet.
...
Das Tragen eines Helmes oder von Arm- oder Beinschienen aus Leder
(auch einzeln) gilt für Belange des Kampfes ohne Waffen wie das Tragen
einer LR. Das Tragen eines entsprechenden Rüstungsteils aus Metall gilt
wie das Tragen von PR (wobei der Gegner eines Faustkämpfers weiterhein
eine vollwertige PR oder besser benötigt, um gegen den Faustkampf
immun zu sein).

Der zweite WW:Abwehr des Opfers eines \emph{Faustkampf}-Angriffs
erhält genau die gleichen WM wie der erste WW:Abwehr \textbf{zuzüglich}
der negativen WM durch den AP-Verlust. Wenn dem Opfer gar kein
WW:Abwehr zustand, erfolgt auch kein zweiter WW:Abwehr. Gelingt der
zweite WW:Abwehr, ist das Opfer auch nicht zu Aktionen fähig, die
ähnliche oder gar höhere Anforderungen an die Konzentrationsfähigkeit
stellen wie ein Angriff, insbesondere \emph{Zaubern}. (Ggf. bereits
laufende Zaubervorgänge werden unterbrochen.) Diese negativen Effekte
setzen auch ein, wenn der zweite WW:Abwehr mißlingt. Für den genauen
Dauer der resultierenden Wehrlosigkeit und Bewußtlosigkeit
vgl. Abschnitt~\ref{timing}.
...
\begin{tabular}{|p{16.5cm}|}
  \hline
  \textbf{Beispiel:} \emph{\small Beowulf beschleunigt sich magisch
...
wird das aus dem Regelwerk nicht sehr deutlich. Es gilt das
...
\item \label{i:zusatz} Zusatzangriffe, die ohne die jeweilige
  ...
  Angriff durch \emph{Fechten}. Hier gilt streng, daß diese Angriffe
  mit keinerlei anderen Spezialregeln kombiniert werden
  können. Allerdings wird ein ggf. in der gleichen Runde
  durchgeführter normaler Angriff davon nicht berührt. Solch ein
  Angriff kann weiterhin durch die speziellen Angriffsweisen aus
  Gruppe \ref{i:ersatz} ersetzt werden. Insbesondere gilt dies für die
  beiden zuletzt genannten Möglichkeiten, die ja nur möglich sind,
  wenn zunächst ein regulärer Angriff durchgeführt wird (wobei dieser
  erste Angriff z.\,B. gezielt sein dürfte -- siehe aber spezielle
  Anmerkungen zum beidhändigen Kampf und \emph{Fechten} [DFR\,140f]).
\item \label{i:ersatz} Spezielle Angriffe, für die die Aktion 2d
  [DFR\,220] (mit einer Nahkampfwaffe angreifen) gewählt werden muß,
  wobei dann aber der übliche normale Angriff durch den speziellen
  Angriff ersetzt wird: gezielter Angriff, Rundumschlag, Entwaffnen
  (nur mit bestimmten Waffen möglich), Mehrfachangriff mit schneller
  Waffe (z.\,B. NunChaku), Anwendung von \emph{Kampf in Dunkelheit},
  \emph{beidhändigem Kampf} bzw. \emph{Kampf in
    Schlachtreihe}. Angriffe, die nicht im Zuge von Aktion 2d
  durchgeführt werden (also z.\,B. aus Gruppe \ref{i:zusatz} oder
  \ref{i:aktion}), können daher nicht mit diesen Angriffsweisen
  kombiniert werden. Weniger offensichtlich ist, daß die
  Angriffsweisen aus dieser Gruppe auch nicht \emph{miteinander}
  kombiniert werden können. Beispiele: Ein Rundumschlag kann nicht
  gezielt ausgeführt werden. Wenn ein beidhändiger Kämpfer im Dunkeln
  kämpft, muß er sich entscheiden, ob er einhändig \emph{Kampf in
    Dunkelheit} anwendet oder lieber mit \emph{beidhändigem Kampf}
  angreift, dafür aber die übliche WM-6 in Kauf nimmt.
\item \label{i:aktion} Angriffe, die eine spezielle Aktionswahl nach
  [DFR\,220f] erfordern: Konzentrierte Abwehr (2f), den Gegner zu Fall
  bringen (2g), ein Handgemenge einleiten (2h). Diese Aktionen sind
  einfach keine normalen Angriffe und können daher nicht durch
  Angriffsweisen aus Gruppe \ref{i:ersatz} ersetzt werden. Während
  einer konzentrierten Abwehr gilt allerdings ausdrücklich, daß durch
  Einsatz von \emph{Fechten} ein Zusatzangriff durchgeführt werden
  kann (siehe Erklärung [DFR\,140]). Analog ist ein Zusatzangriff mit
  einer Parierwaffe gestattet. Andersherum lassen sich die
  Zusatzangriffe aus Gruppe \ref{i:zusatz} aber nicht erschweren, um
  einen Vorteil für die Abwehr zu erhalten.
\end{enumerate}

Zu beachten ist, daß ein \emph{Sturmangriff} keine spezielle
Angriffsweise im Sinne dieses Abschnitts ist. Jeder Nahkampfangriff,
bei dem der Angreifer sich in der laufenden Runde mindestens 6\,m, aber
höchstens die Hälfte seiner Bewegungsweite bewegt hat, zählt
automatisch als Sturmangriff. Ähnliches gilt für Angriffe mit
improvisierten und/oder unvertrauten Waffen. Hier gibt es keinerlei
Einschränkungen bei der Anwendung spezieller Angriffstechniken aus den
...
Zusatzangriffe mit der Parierwaffe hier wieder eine Sonderrolle
spielen und durchaus verdoppelt werden können. Entsprechendes gilt für
den Zauber \emph{Verlangsamen}. 
...
weder ein Rundumschlag noch ein gezielter Hieb möglich.
...
Blickkontakts durch Zwinkern oder wenn eine anderes Wesen zwischen
...
{\small
  \begin{center}
    \begin{tabular}{|c|p{6cm}|p{8cm}|}
      ...
      \end{minipage}\\
      \hline
      2a & kein Angriff möglich (und i.\,d.\,R. auch keine anderen
      Handlungen außer Abwehr -- Handlungsunfähigkeit im eigentlichen Sinne)&
      \begin{minipage}{8cm}
        \vspace*{1mm}
        \begin{itemize}
        ...
        \item Opfer wird im Handgemenge festgehalten (2a und 2b treffen
          hier gleichzeitig zu)
        \end{itemize}
        \vspace*{1mm}
      \end{minipage}\\
      \hline
      ...
      \end{minipage}\\
      \hline
      4 & keine Abwehr, Angreifer erh"alt im Nahkampf WM+4 (diese WM
      gibt es im Handgemenge ohnehin und weiterhin, siehe Stufe 2b)&
      \begin{minipage}{8cm}
        \vspace*{1mm}
        \begin{itemize}
